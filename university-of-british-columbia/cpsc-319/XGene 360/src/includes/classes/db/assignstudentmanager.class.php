...
/* // function called anywhere on earth, can be axed?
	...
	function assign_student_to_problem( $str_student_id, $int_problem_id )
	{
		...
		{			   
			case UP_PROFESSOR:
				...
				$int_number_of_rows = $this->m_obj_db->get_number_of_rows( $obj_problem_manager->view_problem_details( $int_problem_id ) );
			...
			case UP_PROFESSOR:
				...
				if ( $int_number_of_rows != 0 )
				{
					...
					for ( $i = 0; $i < $this->m_obj_db->get_number_of_rows( $res_problem_id ); ++$i )
					{
						...
						$bln_success = $bln_success && $this->assign_student_to_problem ( $res_row->problem_id, $str_student_id );
					}
